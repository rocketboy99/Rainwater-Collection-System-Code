...
// ------------------------------------------------------------------ UI Input and Sensors Initialization ---------------------------------------
   ...
   void Click_Click_Boom(){
    ...
    display.print("TRANSFER_PUMP_RELAY On");
    display.display();
    ...
    Serial.println("System Initializing...");    
    display.setCursor(0, 32);
    Serial.println("DISTRIBUTION_PUMP_RELAY");
    display.setCursor(0, 48);
    display.print("DISTRIBUTION_PUMP_RELAY On");
    display.display();
    ...
    display.setCursor(0, 48);
    display.print("RECIRCULATION_MODE_RELAY On");
    display.display();
    ...
    display.setCursor(0, 48);
    display.print("RECIRCULATION_VALVE_RELAY On");
    display.display();
    ...
    display.setCursor(0, 48);
    display.print("PURGE_VALVE_RELAY On");
    display.display();
    ...
    display.setCursor(0, 48);
    display.print("UV_LIGHT_RELAY On");
    display.display();
...
void determineControlMode() {
    ...
    if(previousControlState == currentControlState){ 
        ...
        currentControlStateIsPreviousState = false;=-koiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii888888888888
    }
...
//turn off the transfer pump, if needed, to prevent exceeding flow rate limit
    if(transferPumpActive == true){
       stopTransferPump();
        ...
        recirculationModeTimeOfRequestedAction = delay();
...
void loop() {
    ...
    // This needs work
    if (recirculationModeActive && (delay() - recirculationModeTimeOfRequestedAction > RECIRCULATION_MODE_TIMEOUT)) {
